...
export function createObservatory(textures={}) {
  ...
  const fineRing=(g,r,t,mat,x=0,y=0,z=0)=>mesh(g,new T.TorusGeometry(r,t,8,48),mat,x,y,z);
  ...
  round(traveler,.44,.24,.08,m.brass,0,1.32,.235,.04);
  round(traveler,.33,.12,.035,m.dark,0,1.34,.283,.025);
  ...
  for(const side of [-1,1]){
    ...
    sphere(arm,.14,m.brass);mesh(arm,new T.CapsuleGeometry(.12,.29,8,20),m.porcelain,0,-.25);
    sphere(arm,.115,m.dark,0,-.47);const elbow=group(arm,0,-.47,0);elbows.push(elbow);
    mesh(elbow,new T.CapsuleGeometry(.105,.24,8,20),m.porcelain,0,-.18);
    sphere(elbow,.12,m.brass,0,-.4);arm.rotation.z=side*.13;
    const leg=group(traveler,side*.175,.85,0);legs.push(leg);
    mesh(leg,new T.CapsuleGeometry(.14,.31,8,20),m.porcelain,0,-.22);
    sphere(leg,.125,m.brass,0,-.44);const knee=group(leg,0,-.44,0);knees.push(knee);
    round(knee,.25,.28,.27,m.porcelain,0,-.14,0,.08);
    round(knee,.29,.18,.43,m.dark,0,-.31,.075,.075);
    round(knee,.29,.035,.43,m.brass,0,-.38,.075,.015);
    ...
    for(let j=0;j<4;j++){
      ...
      round(knee,.255,.012,.03,m.ink,0,-.402,-.05+j*.085,.004);
      mesh(elbow,new T.CapsuleGeometry(.023,.09,3,8),m.porcelain,-.069+j*.046,-.505,.025);
    }
  ...
  for(let i=0;i<5;i++){
    ...
    land.rotation.set(.35+i*.64,i*2.399,.2*i);
  }
  for(let i=0;i<5;i++){const r=ring(planet,1.09,.012,m.brass);r.rotation.y=i*Math.PI/5;}
  ...
  sphere(planet,.13,m.porcelain,1.63,.7,0);
  ...
  const controller=group(games,0,.72,1.5);controller.rotation.x=-.3;
  ...
  round(controller,.26,.075,.055,m.dark,-.62,.12,.24,.025);round(controller,.075,.26,.055,m.dark,-.62,.12,.24,.025);
  ...
  const bookmark=mesh(book,surface((u,v)=>[.62+(v-.5)*.1,.2+.26*Math.sin(u*Math.PI)-u*.46,.6+u*.64],24,3),m.rose);
  bookmark.rotation.y=.08;
  const writtenPage=group(book,0,.22,0);
  for(const side of [-1,1])for(let line=0;line<7;line++){
    const pts=[];for(let j=0;j<12;j++){const u=.2+j*.057;pts.push([side*u*1.43,.25*Math.sin(u*Math.PI*.95)+.08*u+.022,-.63+line*.19]);}tube(writtenPage,pts,.008,m.ink);
  }
  const paperOrbit=group(library,0,1.8,0);
  ring(paperOrbit,2.06,.034,m.brass).rotation.set(.25,.55,.2);
  ring(paperOrbit,2.17,.012,m.glow).rotation.set(.7,-.4,.3);
  for(let i=0;i<5;i++){
    const a=i*1.26;
    const sheet=mesh(paperOrbit,surface((u,v)=>[(u-.5)*.6,.09*Math.sin(u*Math.PI),(v-.5)*.8],16,8),m.paper,Math.cos(a)*1.7,.4+Math.sin(a)*.8,Math.sin(a)*1.3);
  ...
  const deck=mesh(ship,new T.CircleGeometry(1,64),m.brass);deck.rotation.x=-Math.PI/2;deck.scale.y=.39;
  for(let i=-3;i<=3;i++){
